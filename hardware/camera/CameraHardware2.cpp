...
static int faceNotifyCb(int cmd, void * data, void * user)
{
	...
	switch (cmd)
	{
		...
		case FACE_NOTITY_CMD_RESULT:
			...
			camera_hw->getCurrentOriention((int*)(&camera_hw->mFrameFaceData.angle));
			// preview size
...
bool CameraHardware::smartThread()
{
    ...
    if (mSmartDetectionEnable)
    {
		...
		if(ret != 0)
		{
			...
			if (mSmartDetection != NULL)
			{
				mSmartDetection->ioctrl(mSmartDetection, APPERCEIVEPEOPLE_OPS_CMD_START, width, height, mSmartData, mSmartMode);
			}
			else
			{
...
status_t CameraHardware::Initialize()
{
	...
	mSmartDetection->ioctrl(mSmartDetection, APPERCEIVEPEOPLE_OPS_CMD_REGISTE_USER, (int)this, 0, NULL, mSmartMode);
...
status_t CameraHardware::doTakePicture()
{
	...
	if (cur_picture_mode != NULL)
	{
		...
		else if (!strcmp(cur_picture_mode, PICTURE_MODE_SCENE_MODE))
		{
            ...
            int  now_scene_mode = -1;
			if (now_scene_mode_str != NULL)
			{
		        if (!strcmp(now_scene_mode_str, CameraParameters::SCENE_MODE_AUTO)) {
		            now_scene_mode = V4L2_SCENE_MODE_AUTO;
					
		        } else if (!strcmp(now_scene_mode_str, CameraParameters::SCENE_MODE_HDR)){
		            now_scene_mode = V4L2_SCENE_MODE_HDR;
		        }
			}
			
		    if (now_scene_mode == V4L2_SCENE_MODE_HDR)
		    {
				mV4L2CameraDevice->setTakePictureCtrl();
		        mV4L2CameraDevice->setTakePictureState(TAKE_PICTURE_SCENE_MODE);
			    mV4L2CameraDevice->startSceneModePicture(now_scene_mode);
			    return OK;
		    }
		}
	}
...
void CameraHardware::makeFDOrientionArray()
{
	...
	if(mHalCameraInfo.orientation == 90 || mHalCameraInfo.orientation == 270){
		for(int i = 0;i < 4;i++){
			mFDOriention[i] = (i * 90 +360 - mHalCameraInfo.orientation)%360;
...
void CameraHardware::getCurrentOriention(int * oriention)
{
	...
	*oriention = mFDOriention[mOriention];
}
    ...
    if (new_picture_mode != NULL) 
	{
		...
		if (mCameraConfig->supportSceneMode())
		{
				...
				&& (mFirstSetParameters || strcmp(now_scene_mode_str, new_scene_mode_str)))
			{
		        ...
		        if (!strcmp(now_scene_mode_str, CameraParameters::SCENE_MODE_AUTO))
		            new_scene_mode = V4L2_SCENE_MODE_AUTO;
		        else if (!strcmp(now_scene_mode_str, CameraParameters::SCENE_MODE_HDR))
		            new_scene_mode = V4L2_SCENE_MODE_HDR;
		        else {
		            LOGE("ERR(%s):Invalid scene mode(%s)", __FUNCTION__, new_scene_mode_str);
		            ret = UNKNOWN_ERROR;
		        }
				...
				if (now_scene_mode_str != NULL)
				{
			        ...
			        if (!strcmp(scene_mode_str, CameraParameters::SCENE_MODE_AUTO)) {
			            ...
			            if(0 == mV4L2CameraDevice->openSceneMode()) {
	                        mParameters.set(KEY_PICTURE_MODE, PICTURE_MODE_SCENE_MODE);
							const char * new_path = params.get(KEY_SNAP_PATH);
							LOGV("%s : snap new_path : %s", __FUNCTION__, new_path);
							if (new_path != NULL)
