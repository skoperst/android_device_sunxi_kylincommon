...
protected:
	...
	class DoSceneModePictureThread : public Thread {
        V4L2CameraDevice*	mV4l2CameraDevice;
		bool				mRequestExit;
    public:
        DoSceneModePictureThread(V4L2CameraDevice* dev) :
			Thread(false),
			mV4l2CameraDevice(dev),
			mRequestExit(false) {
		}
		void startThread() {
			run("CameraSceneModePictrueThread", PRIORITY_URGENT_DISPLAY);
		}
		void stopThread() {
			mRequestExit = true;
        }
        virtual bool threadLoop() {
			...
			return mV4l2CameraDevice->sceneModePictureThread();
		}
    };
...
private:
	...
	sp<DoSceneModePictureThread>	mSceneModePictureThread;
	pthread_mutex_t 				mSceneModePictureMutex;
	pthread_cond_t					mSceneModePictureCond;
	bool                            mSceneModePictureDone;
	
	int                             mSceneModePicture;
