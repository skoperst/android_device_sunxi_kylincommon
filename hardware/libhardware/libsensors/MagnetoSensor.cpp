...
#define MAG_DATA_NAME    magSensorInfo.sensorName
/*****************************************************************************/

MagnetoSensor::MagnetoSensor(AccelSensor* as)
        : SensorBase(NULL, MAG_DATA_NAME),
        //mEnabled(0),
        mEnCount(0),
        mInputReader(4),
        mAccSensor(as),
...
int MagnetoSensor::read_sensor_calibration_data(int *mx, int *my, int *mz) {
  //unsigned long fd = fopen("/data/system/sensors.dat1", "r");
		if(mkdir("/data/system",0755) != 0){
			ALOGD("%s: can not create /data/system dir or it does exit !", __func__);
		}
...
int MagnetoSensor::setDelay(int32_t handle, int64_t delay_ns)
{
        ...
        int bytes = sprintf(buf, "%lld", delay_ns/1000);
